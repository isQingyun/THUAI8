...
void MoveFollowPath(ICharacterAPI& api, std::vector<std::pair<int32_t, int32_t>> path, int oneStep)
{
    ...
    // if (Distance(playerLocation.first, playerLocation.second, selfinfo->x, selfinfo->y) <= 1000)
    //{
    //     std::srand(std::time(nullptr)); // 生成 0、1、2、3 中的随机数
    //     int randomNumber = std::rand() % 4;
    //     switch (randomNumber) {
    //     case 0: api.MoveRight(1000 / 2.5); break;
    //     case 1: api.MoveLeft(1000 / 2.5); break;
    //     case 2: api.MoveUp(1000 / 2.5); break;
    //     case 3: api.MoveDown(1000 / 2.5); break;
    //     }
